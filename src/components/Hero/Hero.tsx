...
useEffect(() => {
  ...
  if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
    ...
    if (recognitionRef.current) { // Add this null check
  ...
  return () => {
    if (recognitionRef.current) { // This check is already there
    ...
    <div className="w-full bg-gradient-to-r from-blue-900 to-blue-800 p-4 sm:p-8 rounded-lg shadow-xl relative overflow-hidden">
      ...
      {showAnimation && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 flex flex-col items-center max-w-md w-full">
            <div
              className={`w-24 h-24 rounded-full bg-red-100 flex items-center justify-center mb-4 ${isRecording ? "animate-pulse" : ""}`}
            >
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="mb-6 sm:mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between">
          <h1 className="text-2xl sm:text-3xl font-bold text-white mb-4 sm:mb-0">Looking for a test?</h1>
        ...
        <div className="relative mb-6 sm:mb-8">
          <div
            className={`bg-white rounded-full shadow-lg overflow-hidden flex items-center transition-all duration-300 ${isSearchFocused ? "ring-4 ring-blue-400" : ""}`}
          >
            <Search
              className={`ml-3 sm:ml-4 transition-colors duration-300 ${isSearchFocused ? "text-blue-500" : "text-gray-500"}`}
              size={18}
            />
            <input
              type="text"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
            ...
            <div className="bg-white rounded-xl p-4 sm:p-6 shadow-lg transition-all duration-300 hover:shadow-xl hover:-translate-y-1 cursor-pointer group">
              ...
              <div className="mt-3 sm:mt-4 flex justify-between items-center">
                <p className="text-gray-500 text-xs sm:text-sm pr-2">
                  Quick results based on your doctor's recommendations
                </p>
          ...
          <div className="bg-white rounded-xl p-4 sm:p-6 shadow-lg transition-all duration-300 hover:shadow-xl hover:-translate-y-1 cursor-pointer group">
            ...
            <div className="mt-3 sm:mt-4 flex justify-between items-center">
              <p className="text-gray-500 text-xs sm:text-sm pr-2">
                Choose from our expert-designed health checkup packages
              </p>
              <div className="w-12 h-12 sm:w-16 sm:h-16 bg-blue-50 rounded-full flex-shrink-0 flex items-center justify-center">
